...
class OwnerOldActivityInfoScreen extends Component {
  ...
  joinYes = async (rating: number) => {
    ...
    console.log('memberCollection', this.state.selectedMember);
    const selectedMember = memberCollection.docs
      .filter(
        (item) =>
          item.data().memberEmail === this.state.selectedMember.memberEmail
      )[0]
      .data();
      ...
      return (
        ...
        >
          ...
          <Button
            title="No"
            color={
              member._data.memberJoin === false && member._data.ownerState
                ? 'red'
                : '#CCC'
            }
          ...
          <Button
            title="Yes"
            color={
              member._data.memberJoin === true && member._data.ownerState
                ? '#37CC4A'
                : '#CCC'
            }
            onPress={() => this.buttonYesAction(member)}
          />
        </View>
      );
    };
...
export default OwnerOldActivityInfoScreen;
